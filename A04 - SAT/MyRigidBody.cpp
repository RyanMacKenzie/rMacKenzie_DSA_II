...
{
	//I don't like the book's variable names any more than the openGL standard
	float ra, rb;
	Simplex::matrix3 R, AbsR;

	// Other's rotation compared to this object's
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			R[i][j] = glm::dot(m_m4ToWorld[i], a_pOther->GetModelMatrix()[j]);

	// Get vector between centers
	Simplex::vector3 t = a_pOther->GetCenterGlobal() - m_v3Center;
	// The same translation in this object's local coordinates
	t = Simplex::vector3(glm::dot(t, Simplex::vector3(m_m4ToWorld[0])), glm::dot(t, Simplex::vector3(m_m4ToWorld[1])), glm::dot(t, Simplex::vector3(m_m4ToWorld[2])));

	// make a version of the rotation matrix where no value is 0
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			AbsR[i][j] = glm::abs(R[i][j]) + 0.00001;
	
	//massive amount of axis tests. that is to say, the algorithm's expensive part.
	...
	// none of the axis tests found anything, so we're intersecting
	return 1;
}