...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	//your code goes here---------------------
	vector3 m_v3ARBBArray[8]; //I don't actually understand the m_ part of the variable naming convention
	m_v3ARBBArray[0] = m_v3MinL; //actually shove shove in every possible combination of points derived from the two corners
	m_v3ARBBArray[1] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MinL.z);
	m_v3ARBBArray[2] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MaxL.z);
	m_v3ARBBArray[3] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MinL.z);
	m_v3ARBBArray[4] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MinL.z);
	m_v3ARBBArray[5] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MaxL.z);
	m_v3ARBBArray[6] = vector3(m_v3MinL.x, m_v3MinL.y, m_v3MaxL.z);
	m_v3ARBBArray[7] = m_v3MaxL;

	for (int i = 0; i < 8; ++i) //convert to world space using voodoo magic matrix multiplication
	{
		m_v3ARBBArray[i] = vector3(m_m4ToWorld * vector4(m_v3ARBBArray[i], 1.0f));
	}

	m_v3MaxG = m_v3ARBBArray[0]; //new globals... for now...
	m_v3MinG = m_v3ARBBArray[0];

	for (int i = 1; i < 8; ++i) //prepare to expand global points to their true form
	{
		if (m_v3MaxG.x < m_v3ARBBArray[i].x)
		{
			m_v3MaxG.x = m_v3ARBBArray[i].x;
		}
		else if (m_v3MinG.x > m_v3ARBBArray[i].x)
		{
			m_v3MinG.x = m_v3ARBBArray[i].x;
		}

		if (m_v3MaxG.y < m_v3ARBBArray[i].y)
		{
			m_v3MaxG.y = m_v3ARBBArray[i].y;
		}
		else if (m_v3MinG.y > m_v3ARBBArray[i].y)
		{
			m_v3MinG.y = m_v3ARBBArray[i].y;
		}

		if (m_v3MaxG.z < m_v3ARBBArray[i].z)
		{
			m_v3MaxG.z = m_v3ARBBArray[i].z;
		}
		else if (m_v3MinG.z > m_v3ARBBArray[i].z)
		{
			m_v3MinG.z = m_v3ARBBArray[i].z;
		}
	} //yeehaw. I almost understand how this works.
	//----------------------------------------

	//we calculate the distance between min and max vectors
	m_v3ARBBSize = m_v3MaxG - m_v3MinG; //this was the whole goal all along, huh...
}
//The big 3
