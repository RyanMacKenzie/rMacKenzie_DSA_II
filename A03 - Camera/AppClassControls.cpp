...
void Application::CameraRotation(float a_fSpeed)
{
	...
	//hold on these angles aren't actually that useful as they are...
	float fAngleX = 0.0f;
	float fAngleY = 0.0f;
	...
	//global variables used mostly for assisting in translation later- which doesn't work because the rotation code below doesn't work
	fTotalAngleX += fAngleX;
	fTotalAngleY += fAngleY;

	//Change the Yaw and the Pitch of the camera
	//If only ANY of the code in this solution was readable I'd have an idea how to do quaternions here... What in the name of all that is holy is that Joystick method down there? ArcBall means nothing to me
	vector3 currentPosition = m_pCamera->GetPosition();
	vector3 targetFromOrigin = normalize(m_pCamera->GetTarget() - currentPosition);
	vector3 aboveFromOrigin = normalize(m_pCamera->GetAbove() - currentPosition);
	targetFromOrigin = rotateX(targetFromOrigin, fAngleX);
	targetFromOrigin = rotateY(targetFromOrigin, fAngleY);
	aboveFromOrigin = rotateX(targetFromOrigin, fAngleX);
	aboveFromOrigin = rotateY(targetFromOrigin, fAngleY);

	m_pCamera->SetPositionTargetAndUpward(currentPosition, targetFromOrigin + currentPosition, aboveFromOrigin + currentPosition);
	//doesn't work because it's using a value derived from local coordinates to move in the global scale
	//can I go back to C# and Unity? That's eldritch but at least it's fun

	/*vector3 target = m_pCamera->GetTarget();
	vector3 right = glm::normalize(glm::cross(m_pCamera->GetAbove(), m_pCamera->GetTarget()));
	vector3 up = m_pCamera->GetAbove();
	glm::mat4 view;
	view = glm::mat4(right.x, right.y, right.z, 0,
...
#pragma region Camera Position
	...
	//initiating variable to save line space and my poor legally-blind eyes later
	vector3 currentPosition = m_pCamera->GetPosition();
	vector3 currentTarget = m_pCamera->GetTarget();
	vector3 currentAbove = m_pCamera->GetAbove();

	
	if (sf::Keyboard::isKeyPressed(sf::Keyboard::W)) //don't know why W and S were already coded with the MoveForward method but I assume that's not what I'm supposed to use so I'll leave them like this
	{
		//Doing each of these separately instead of in the method that does all 3 at once because, frankly, it's easier to read this way, and I am dumb
		//Rotate the transformation by globally-tracked camera angles. Slightly fucked because the camera angle calculations are slightly fucked.
		m_pCamera->SetPosition(vector3(currentPosition.x, currentPosition.y, currentPosition.z) + rotateX(rotateY(vector3(0, 0, -fSpeed), fTotalAngleY), fTotalAngleX));
		m_pCamera->SetTarget(vector3(currentTarget.x, currentTarget.y, currentTarget.z) + rotateX(rotateY(vector3(0,0,-fSpeed), fTotalAngleY), fTotalAngleX));
		m_pCamera->SetAbove(vector3(currentAbove.x, currentAbove.y, currentAbove.z) + rotateX(rotateY(vector3(0, 0, -fSpeed), fTotalAngleY), fTotalAngleX));
		//m_pCamera->SetPosition(currentPosition + (fSpeed * currentTarget));
	}
	...
	{
		m_pCamera->SetPosition(vector3(currentPosition.x, currentPosition.y, currentPosition.z) + rotateX(rotateY(vector3(-fSpeed, 0, 0), fTotalAngleY), fTotalAngleX));
		m_pCamera->SetTarget(vector3(currentTarget.x, currentTarget.y, currentTarget.z) + rotateX(rotateY(vector3(-fSpeed, 0, 0), fTotalAngleY), fTotalAngleX));
		m_pCamera->SetAbove(vector3(currentAbove.x, currentAbove.y, currentAbove.z) + rotateX(rotateY(vector3(-fSpeed, 0, 0), fTotalAngleY), fTotalAngleX));
	}
